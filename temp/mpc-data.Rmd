...
lineData <- p1Data %>% 
  group_by(y) %>% 
  arrange(x, .by_group=TRUE) %>%  #first group by y and then based on that group by x?
  summarize(line = paste(text, collapse=" "))
...
lineData <- p1Data %>% 
  ...
  summarize(line = paste(text, collapse=" ")) #paste all words in the same line together
```
...
#achieve the maximum column position but will also include text in it
...
#countData merge things of the same row together
...
t(pdfMPC.page1("Prince Georges County Statistical Report 2018.pdf"))
...
```
```{r}
myData[myData$County=="Allegany County, MD"]
```
