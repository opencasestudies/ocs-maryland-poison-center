...
`map_data()` in package ggplot2, which turns a series of points along an outline into a data frame of those points.
...
5.group. This is very important! ggplot2's functions can take a group argument which controls (amongst other things) whether adjacent points should be connected by lines. If they are in the same group, then they get connected, but if they are in different groups then they don't.
...
Everytime talking about plot, ggplot2 appears in our mind. In this case, it also contains some pretty useful functions that make it easy to interact with the data in `maps` package.  `geom_polygon()` draws lines between points,
...
ggplot() + 
  ...
  geom_polygon(data = state.md, aes(x=long, y=lat, group=group), color="black", fill=NA) +  #outline for state
  coord_fixed(1.3) + 
  theme_bw()
...
First I will make the plots faceted, if every y unit was 1.3 times longer than an x unit, then the plot came out looking good.
