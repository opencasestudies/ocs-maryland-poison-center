...
from 2006 to 2008 of all counties (totally 312 files). 
...
As we mentioned over and over, one common probelm 
...
'ReasonUn:Outcome', which only has one value:
...
wired format of original pdf, just delete it
...
a group argument which controls (amongst other things) 
whether adjacent points should be connected by lines. 
...
after removing the artificially low rates for these 
two counties, we are also able to see more differentiation 
...
baseMap <- ggplot() + 
  ...
  coord_fixed(1.3) + scale_fill_gradient(low = "white", 
               high = "purple", na.value = "grey80") +
  theme_void() + theme(legend.position = c(.92,.7), 
              legend.text.align = 0, plot.title = 
