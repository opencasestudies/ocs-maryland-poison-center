...
Then just combine this dataframe with information for year and county:
...
The final dataset we could get contains `r nrow(D1)` and `r ncol(D1)` columns.
...
To achieve this, we only need to update the group function:
```{r}
...
cd <- cd %>%
  mutate(variableSub=ifelse(column=="Left" & y > y.age.min & y < y.age.max, "Age:",
         ifelse(column=="Left" & y > y.un.min & y < y.un.max, "RFE-Unintent:",
         ifelse(column=="Left" & y > y.int.min & y < y.int.max, "RFE-Intent:",
         ifelse(column=="Left" & y > y.other.min, "RFE-Other:",
